...
function requestParams(argv, templates) {
	...
	return [
		...
		{
			type: prev => (!argv.npm && prev ? 'confirm' : null),
			name: 'npm',
			message: 'Install with `npm` instead of `yarn`',
			initial: false,
		},
		{
...
module.exports = async function(repo, dest, argv) {
	...
	if (!repo || !dest) {
		...
		info('Alternatively, run `oreact init --help` for usage info.');
		return;
	}

	let cwd = resolve(argv.cwd);
	let target = resolve(cwd, dest);
	let isYarn = !argv.npm && hasCommand('yarn');
	...
	// Copy env file
	const sourceDirectory = resolve(cwd, dest);
	// Copy over template.html
	const templateSrc = resolve(
		__dirname,
		join('..', 'resources', '.env')
	);
	const templateDest = join(sourceDirectory, '.env');
	keeps.push(templateDest);
