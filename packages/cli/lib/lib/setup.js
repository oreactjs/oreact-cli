...
exports.addScripts = async function(obj, cwd, isYarn) {
	...
	return {
		build: 'oreact build',
		serve: 'oreact build && oreact serve',
...
exports.initGit = async function(target) {
	...
	if (git) {
		...
		const defaultGitUser = 'Oreact CLI';
		const defaultGitEmail = 'oreact-cli@users.noreply.github.com';
		...
		await spawn('git', ['commit', '-m', 'initial commit from Oreact CLI'], {
			cwd,
			env: {
